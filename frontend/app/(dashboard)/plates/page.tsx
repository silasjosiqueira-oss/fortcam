  ...
  return (
    <div style={{ padding:16 }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#0d1520,#0a1018)", border:"1px solid rgba(0,160,255,0.15)", borderRadius:10, overflow:"hidden" }}>
        <div style={{ display:"grid", gridTemplateColumns:"36px 90px 140px 1fr 180px 120px", padding:"8px 14px", borderBottom:"1px solid rgba(0,160,255,0.1)", fontSize:10, color:"#4a6a8a", textTransform:"uppercase" }}>
        ...
        {filtrados.map(ev => {
          ...
          return (
            <div key={ev.id} style={{ display:"grid", gridTemplateColumns:"36px 90px 140px 1fr 180px 120px", padding:"8px 14px", borderBottom:"1px solid rgba(255,255,255,0.03)", alignItems:"center" }}>
              ...
              <div>
                <button onClick={()=>verFoto(ev)} style={{ width:70, height:40, borderRadius:4, background:"rgba(0,100,200,0.15)", border:"1px solid rgba(0,160,255,0.2)", cursor:"pointer", color:"#7ec8ff", fontSize:11, display:"flex", alignItems:"center", justifyContent:"center", gap:4 }}>
                  📷 Ver
                </button>
              </div>
      ...
      {fotoModal && (
        <div onClick={()=>setFotoModal(null)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.92)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100, cursor:"pointer" }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:"linear-gradient(135deg,#0d1520,#0a1018)", border:"1px solid rgba(0,160,255,0.25)", borderRadius:12, padding:20, maxWidth:700, width:"90%" }}>
            ...
            {fotoLoading && <div style={{ height:200, display:"flex", alignItems:"center", justifyContent:"center", color:"#4a6a8a" }}>Carregando foto...</div>}

            {!fotoLoading && fotoModal.image_b64 && (
              <img
                src={fotoModal.image_b64.startsWith("data:") ? fotoModal.image_b64 : `data:image/jpeg;base64,${fotoModal.image_b64}`}
                style={{ width:"100%", borderRadius:8, border:"1px solid rgba(0,160,255,0.15)", maxHeight:500, objectFit:"contain" }}
              />
            )}

            {!fotoLoading && !fotoModal.image_b64 && (
              <div style={{ height:200, display:"flex", alignItems:"center", justifyContent:"center", color:"#4a6a8a", fontSize:13 }}>Sem foto disponivel</div>
            )}

            <div style={{ marginTop:12, fontSize:11, color:"#5a7a9a", fontFamily:"'Share Tech Mono',monospace", display:"flex", justifyContent:"space-between" }}>
              <span>Camera: {fotoModal.camera_name}</span>
              <span>{new Date(fotoModal.detected_at).toLocaleString("pt-BR")}</span>
            </div>
            <button onClick={()=>setFotoModal(null)} style={{ width:"100%", marginTop:14, padding:10, border:"1px solid rgba(255,255,255,0.1)", borderRadius:6, background:"transparent", color:"#5a7a9a", cursor:"pointer" }}>Fechar</button>
          </div>
        </div>
      )}
    </div>
