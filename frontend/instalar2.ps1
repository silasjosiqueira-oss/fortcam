# Script Fortcam Cloud - SEM EMOJIS para evitar problemas de encoding
...
# PAGE.TSX RAIZ
[System.IO.File]::WriteAllText("$base\app\page.tsx", @'
...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setTimeout(() => {
      ...
      else { setError("Preencha o e-mail e a senha."); setLoading(false); }
    }, 1000);
  }
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#080d14 0%,#0a1525 50%,#080d14 100%)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Rajdhani',sans-serif", overflow:"hidden", position:"relative" }}>
      ...
      <div style={{ width:420, background:"linear-gradient(135deg,#0d1520,#0a1018)", border:"1px solid rgba(0,160,255,0.2)", borderRadius:16, padding:"40px 36px", position:"relative", boxShadow:"0 0 60px rgba(0,100,200,0.15)" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:32 }}>
          <div style={{ width:64, height:64, background:"linear-gradient(135deg,#0066cc,#003d7a)", borderRadius:16, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px", boxShadow:"0 0 24px rgba(0,100,200,0.4)", fontSize:28, color:"#7ec8ff", fontWeight:900, fontFamily:"'Orbitron',monospace" }}>FC</div>
        ...
        <form onSubmit={handleLogin}>
          <div style={{ marginBottom:16 }}>
            ...
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="seu@email.com"
              style={{ width:"100%", padding:"10px 14px", background:"rgba(0,0,0,0.3)", border:"1px solid rgba(0,160,255,0.2)", borderRadius:8, color:"#e0e8f0", fontSize:14, outline:"none", fontFamily:"'Rajdhani',sans-serif" }}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display:"flex", height:"100vh", width:"100vw", fontFamily:"'Rajdhani',sans-serif", background:"#080d14", color:"#e0e8f0", overflow:"hidden" }}>
      ...
      {/* SIDEBAR */}
      <div style={{ width:190, flexShrink:0, background:"linear-gradient(180deg,#0a0f1a,#080d14)", borderRight:"1px solid rgba(0,160,255,0.15)", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
        <div style={{ height:52, background:"rgba(8,13,20,0.95)", borderBottom:"1px solid rgba(0,160,255,0.1)", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 20px" }}>
          ...
          <div style={{ width:32, height:32, borderRadius:"50%", background:"linear-gradient(135deg,#0066cc,#003d7a)", display:"flex", alignItems:"center", justifyContent:"center", color:"#7ec8ff", fontWeight:700, fontSize:12, fontFamily:"'Orbitron',monospace" }}>AD</div>
        </div>
  ...
  return (
    <div style={{ padding:16, display:"flex", flexDirection:"column", gap:12 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
        <Card label="Acessos Hoje" valor="128" cor="#00bfff" />
        <Card label="Cameras Ativas" valor="12" cor="#00e676" sub="Online" />
        <Card label="Whitelist Hoje" valor="87" cor="#00bfff" />
        <Card label="Alertas" valor="3" cor="#ff6b35" sub="Avisos" />
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 280px", gap:12 }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#0d1520,#0a1018)", border:"1px solid rgba(0,160,255,0.15)", borderRadius:10, overflow:"hidden" }}>
          ...
          <div style={{ position:"relative", height:160, background:"linear-gradient(135deg,#0a1a2a,#0d2035)", display:"flex", alignItems:"center", justifyContent:"center" }}>
            ...
            <div style={{ fontSize:60, opacity:0.1, color:"#00bfff" }}>[CAM]</div>
        ...
        {eventos.map(ev=>{
          ...
          return (
            <div key={ev.id} style={{ display:"grid", gridTemplateColumns:"36px 130px 60px 1fr 140px 140px", padding:"8px 14px", borderBottom:"1px solid rgba(255,255,255,0.03)", alignItems:"center" }}>
              ...
              <span style={{ background:"#009c3b", color:"#fff", fontSize:9, fontWeight:700, padding:"2px 5px", borderRadius:3, display:"inline-block" }}>{ev.estado}</span>
              ...
              <span style={{ color:ok?"#00e676":"#ff4444", fontSize:11, fontWeight:600, background:ok?"rgba(0,230,118,0.1)":"rgba(255,68,68,0.1)", padding:"2px 8px", borderRadius:4, border:`1px solid ${ok?"rgba(0,230,118,0.3)":"rgba(255,68,68,0.3)"}`, display:"inline-block" }}>{ev.status}</span>
            </div>
          );
        })}
      </div>
        ...
        <div style={{ display:"flex", gap:12, padding:14 }}>
          {cameras.map(cam=>(
            <div key={cam.id} style={{ flex:1, borderRadius:8, overflow:"hidden", border:"1px solid rgba(255,255,255,0.08)", background:"#0a0f1a", position:"relative" }}>
              <div style={{ height:90, background:cam.status==="online"?"linear-gradient(135deg,#0d1b2a,#1a2a3a)":"linear-gradient(135deg,#111,#1a1a1a)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, color:cam.status==="online"?"rgba(0,180,255,0.5)":"rgba(255,255,255,0.1)", fontFamily:"'Orbitron',monospace", letterSpacing:1 }}>{cam.status==="online"?"[LIVE]":"[OFF]"}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:12 }}>
        {cameras.map(cam=>(
          <div key={cam.id} onClick={()=>setSel(sel===cam.id?null:cam.id)} style={{ background:"linear-gradient(135deg,#0d1520,#0a1018)", border:`1px solid ${sel===cam.id?"rgba(0,160,255,0.4)":"rgba(0,160,255,0.15)"}`, borderRadius:10, overflow:"hidden", cursor:"pointer", boxShadow:sel===cam.id?"0 0 20px rgba(0,100,200,0.2)":"none", transition:"all 0.2s" }}>
            <div style={{ height:160, background:cam.status==="online"?"linear-gradient(135deg,#0a1a2a,#0d2035)":"linear-gradient(135deg,#111,#0a0a0a)", display:"flex", alignItems:"center", justifyContent:"center", position:"relative", fontSize:13, color:cam.status==="online"?"rgba(0,180,255,0.5)":"rgba(255,255,255,0.1)", fontFamily:"'Orbitron',monospace", letterSpacing:2 }}>
              {cam.status==="online"?"[CAMERA LIVE]":"[OFFLINE]"}
              {cam.status==="online" && (
                <div style={{ position:"absolute", top:10, right:10, display:"flex", alignItems:"center", gap:4, background:"rgba(0,0,0,0.6)", padding:"3px 8px", borderRadius:4, border:"1px solid rgba(255,50,50,0.4)" }}>
                  <div style={{ width:6, height:6, borderRadius:"50%", background:"#ff3333", boxShadow:"0 0 6px #ff3333", animation:"blink 1.5s ease infinite" }} />
                  <span style={{ fontSize:9, color:"#ff7777", fontWeight:700, letterSpacing:1, fontFamily:"'Orbitron',monospace" }}>AO VIVO</span>
